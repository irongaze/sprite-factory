# Master class that manages editing process internals
@tool
...
signal _channel_changed(channel: Sprite.Channel)
...
signal _color_changed(color: Color, channel: Sprite.Channel)
...
static var channel := Sprite.Channel.DIFFUSE
...
static func select_channel(new_channel: Sprite.Channel):
  # Reset our selection
  selection.clear()
...
static func closest_point(pt):
  if model == null: return null
  var closest = null
  for ch in Sprite.Channel.values():
